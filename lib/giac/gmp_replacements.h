...
inline void mpz_init_set_si(mpz_t &  a,long b){ 
  mp_init_ul(&a,b>0?b:-b); //EDIT: MOV-RAX
  if (b<0) mp_neg(&a,&a);}
inline int mpz_init_set_ui(mpz_t &  a,unsigned long b){ 
  return mp_init_ul(&a,b);  //EDIT: MOV-RAX
  }
...
inline void mpz_set_ui(mpz_t &  a,unsigned int ui){ 
  mp_set_ul(&a,ui); //EDIT: MOV-RAX
  }
inline void mpz_set_si(mpz_t &  a,int ui){ 
  mp_set_ul(&a,ui>0?ui:-ui); //EDIT: MOV-RAX
  if (ui<0) mp_neg(&a,&a); }
...
inline int mpz_pow_ui(mpz_t & c,const mpz_t & a,unsigned int B){ 
  return mp_expt_n((mp_int *)&a,B,&c); // EDIT: MOV-RAX
  }
inline void mpz_ui_pow_ui(mpz_t & c,unsigned int A,unsigned int B){ 
  mp_int a ; 
  mp_init_ul(&a,A); // EDIT: MOV-RAX
  mp_expt_n(&a,B,&c); // EDIT: MOV-RAX
  mp_clear(&a);}
...
inline int mpz_sizeinbase(const mpz_t & a,int radix){ 
  size_t size; //EDIT: MOV-RAX
  mp_radix_size((mp_int *)&a,radix,&size); 
  return size; }
inline int mpz_get_si(const mpz_t & a){
#if 1
  if (a.sign!=MP_NEG)
    return mp_get_u32((mp_int *)&a);
  mp_int * ptr=(mp_int *)&a;
  mp_neg(ptr,ptr);
  long u=mp_get_u32(ptr);
  mp_neg(ptr,ptr);
  return -u;
#else
  mp_int tmp,res;
...
inline int mpz_get_str(char * s,int base, size_t maxlen, const mpz_t &  z){ // EDIT: MOV-RAX
  return mp_to_radix((mp_int *)&z,s,maxlen, NULL, base);
  }
inline double mpz_get_d(const mpz_t & z){ 
  ...
  char s[512]; 
  mp_to_radix((mp_int *)&z,s,sizeof(s),NULL,10);
...
inline bool mpz_perfect_square_p(const mpz_t & a){
  bool res; //EDIT: MOV-RAX
  mp_is_square((mp_int *)&a,&res);
  return res; //EDIT: MOV-RAX
}
